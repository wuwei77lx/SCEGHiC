...
knitr::opts_knit$set(root.dir = "/mnt/netshare2/liangxuan/data/model/model/data/human/pbmc")
...
weight <- calculateHiCWeights(SCEGdata, species = "Homo sapiens", genome = "hg38", focus_gene = gene, averHicPath = "/mnt/netshare2/liangxuan/data/human_contact/AvgHiC")
```
...
weight <- calculateHiCWeights(SCEGdata, species = "Homo sapiens", genome = "hg38", focus_gene = "PDK1", averHicPath = "/mnt/netshare2/liangxuan/data/human_contact/AvgHiC")
```
...
Here, the ground truth for CD4+ T cells is obtained from the [ENCODE database](https://www.encodeproject.org/files/ENCFF762HKR/@@download/ENCFF762HKR.hic), and the correlation is calculated as the Pearson correlation coefficient between each gene and its candidate enhancers. The detailed code for this analysis is publicly available at the [GitHub repository](https://github.com/wuwei77lx/compare_model).
...
# Load the truth of CD4+ T data
truth_CD4 <- readRDS("truth_CD4.rds")

## Load correlation data
correlation <- readRDS("correlation_CD4.rds")

# Coverage plot and visualize the links of PDK1
coverPlot(pbmc, focus_gene = "PDK1", species = "Homo sapiens", genome = "hg38", assay = "peaks", HIC_Result = truth_CD4, SCEG_HiC_Result = results_SCEGHiC, SCEG_HiC_cutoff = 0.01, correlation = correlation, cells = c("CD4 T", "CD8 T"))
```
