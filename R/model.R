...
#' @param alpha Multiplicative scaling factor for the penalty parameter (alpha * rho).
#'
...
#' @export
#'
wglasso <- function(S, sample, normhicMatrix, element, alpha = NULL) {
  # Add a small constant to the diagonal to exclude self-loop edges
  diag(S) <- diag(S) + 1e-4
  ...
  # alpha: multiplicative scaling factor for the penalty parameter (alpha * rho)
  if (!is.null(alpha)) {
    rho <- rho * alpha
  }
  ...
  print(paste("The optimal penalty parameter (rho) selected by BIC is:", rho))
...
#'
Run_SCEG_HiC <- function(SCEGdata, HiCWeights, method = "wglasso", focus_gene, normalizeMethod = "1", cutoff = NULL, verbose = TRUE, alpha = NULL) {
  ...
  for (gene in focus_gene) {
    ...
    if (sd(Y) > 0) {
      ...
      if (method == "wglasso") {
        result_matrix <- wglasso(s, sample = ncol(atac), normhicMatrix = genecontact, element = colnames(X2), alpha = NULL)
      } else {
