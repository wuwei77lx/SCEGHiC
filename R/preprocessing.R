...
#' \code{Aggregate_datasets} function.
#'
#' @param object A Seurat object.
#' @param k_neigh Integer. Number of cells to aggregate per group (default is 50).
#' @param rna_assay Character. Name of the assay containing gene expression data.
#' @param atac_assay Character. Name of the assay containing peak (chromatin accessibility) data.
...
#' @param atacbinary Logical. Should the aggregated scATAC-seq matrix be binarized?
#' @param max_overlap Numeric. Maximum allowed overlap ratio between two aggregated groups (default is 0.8).
#' @param reduction.name Character. Name of the dimensionality reduction to use for extracting cell coordinates for aggregation.
#' @param size_factor_normalize Logical. Whether to normalize accessibility values using size factors.
...
Aggregate_datasets <- function(object, k_neigh = 50, rna_assay = "RNA",
  ...
  reduction_name <- ifelse(is.null(reduction.name), "umap", reduction.name)
  cell_coord <- object@reductions[[reduction_name]]@cell.embeddings
  ...
  for (i in 1:length(uniqgroup)) {
    ...
    if (length(sub_index) < k_neigh) {
      if (verbose) message("Skipping group ", uniqgroup[i], " (not enough cells)")
      next
    }

    cell_coord_i <- cell_coord[sub_index, , drop = FALSE]

    # Generate aggregated data
    sub_aggregated_datasets <- generate_aggregated_datasets(subobject, cell_coord_i, rna_assay, atac_assay, k_neigh, atacbinary, max_overlap, seed, verbose)

    sub_cell_sample <- sub_aggregated_datasets$cell_sample
    message(paste0("There are ", dim(sub_cell_sample)[1], " samples"))
...
#'
#' Function to generate aggregated inputs of a cetrain cluster. \code{generate_aggregated_datasets}
...
#'
#' @param object A Seurat object.
#' @param cell_coord A similarity matrix or dimensionality reduction (e.g., PCA, UMAP) used for identifying neighbors.
#' @param rna_assay Character. Name of the assay containing gene expression data.
#' @param atac_assay Character. Name of the assay containing peak (chromatin accessibility) data.
#' @param k_neigh Integer. Number of neighboring cells to aggregate per group (default is 50).
#' @param atacbinary Logical. Should the aggregated scATAC-seq matrix be binarized?
#' @param max_overlap Numeric. Maximum allowed overlap ratio between two aggregated groups (default is 0.8).
#' @param seed Integer. Random seed.
#' @param verbose Logical. Logical. Should progress messages and warnings be printed?
#'
#' @importFrom FNN knn.index
#' @import Matrix
#'
#' @return A matrix or sparse matrix containing aggregated accessibility or expression values.
#' @export
#'
generate_aggregated_datasets <- function(object, cell_coord, rna_assay = "RNA", atac_assay = "peaks", k_neigh = 50, atacbinary = TRUE, max_overlap = 0.8, seed = 123, verbose = TRUE) {
...
#'
#' @param object A Seurat object.
#' @param aggregate Logical. Whether to generate aggregated (clustered) data.
#' If \code{FALSE}, a specific \code{celltype} must be provided.
#' @param celltype Character. A specific cell type to subset for non-aggregated data.
...
#' @param atac_assay Character. Name of the assay containing chromatin accessibility data.
...
#' @import Seurat
#' @import Matrix
#' @return A list containing preprocessed data for SCEG-HiC:
#' \itemize{
...
process_data <- function(object, aggregate = TRUE, celltype = NULL, rna_assay = "RNA", atac_assay = "peaks", cellnames = NULL, atacbinary = TRUE, k_neigh = 50, max_overlap = 0.8, size_factor_normalize = TRUE, reduction.name = NULL, seed = 123, verbose = TRUE) {
  if (aggregate) {
    ...
    if (rna_assay %in% names(object@assays)) {
      if (inherits(dataset@assays[[rna_assay]], "Assay5")) {
        rna <- dataset@assays[[rna_assay]]@layers[["data"]]
        rownames(rna) <- rownames(dataset@assays[[rna_assay]])
